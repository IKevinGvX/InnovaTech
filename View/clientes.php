...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['crear'])) {
        if (
            ...
            $error = "Por favor, complete todos los campos para agregar un cliente";
        }
        ...
        if (!empty($_POST['cliente_id'])) {
            ...
            $error = "Error al intentar eliminar el cliente";
        }
    }
}
...
<body>
    ...
    <header>
        <h1>Gestión de Clientes - INNOVATECH</h1>
            ...
            <table id="clientestable" class="display">
                ...
                <tbody>
                    <?php while ($cliente = $clientes->fetch_assoc()): ?>
                        <tr>
                            <td><?= $cliente['cliente_id'] ?? 'Sin ID' ?></td>
                            <td><?= $cliente['usuario_id'] ?? 'Sin Usuario ID' ?></td>
                            <td><?= $cliente['nombre'] ?? 'Sin Nombre' ?></td>
                            <td><?= $cliente['apellido'] ?? 'Sin Apellido' ?></td>
                            <td><?= $cliente['fecha_nac'] ?? 'Sin Fecha de Nacimiento' ?></td>
                            <td><?= $cliente['dni'] ?? 'Sin DNI' ?></td>
                            <td><?= $cliente['ruc'] ?? 'Sin RUC' ?></td>
                            <td><?= $cliente['correo'] ?? 'Sin Correo' ?></td>
                            <td><?= $cliente['telefono'] ?? 'Sin Teléfono' ?></td>
                            <td><?= $cliente['direccion'] ?? 'Sin Dirección' ?></td>
                            <td>
                                <a href="editarclientes.php?id=<?= $cliente['cliente_id'] ?>" class="btn-fire">Editar</a>
